...
YT_API_KEY = "xbit_q1Wl-cXMPdYy_GGP0LpxQ0e9FmV2KkPW"
...
async def download_proxy_engine(vid_id: str, title: str, is_video: bool) -> str:
    ...
    try:
        ...
        if data.get('status') == 'success':
            ...
            if not url:
                logger.error("API Error: Owner succeeded but cache empty")
                return None
...
async def download_ytdlp_engine(link: str, vid_id: str, title: str, is_video: bool) -> str:
    """Direct yt-dlp fallback — used when all API engines fail."""
    ext = "mp4" if is_video else "mp3"
    fname = f"{title}.{ext}" if title else f"{vid_id}.{ext}"
    path = os.path.join("downloads", fname)
    ...
    try:
        ...
        def _dl():
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if not info:
                    raise ValueError("Age check passed but no streaming data returned")
        await loop.run_in_executor(None, _dl)
        # yt-dlp may add extension suffix
        if not os.path.exists(path):
            alt = path.replace(f".{ext}", f".{ext}.{ext}")
            if os.path.exists(alt):
                os.rename(alt, path)
        return path if os.path.exists(path) else None
    except Exception as e:
        logger.error(f"yt-dlp engine error: {e}")
        if os.path.exists(path):
            try: os.remove(path)
            except: pass
        return None
...
class YouTubeAPI:
    ...
    async def download(
        ...
        # All API engines failed — fall back to direct yt-dlp
        logger.warning(f"API engines failed for {vid_id}, falling back to yt-dlp")
        result = await download_ytdlp_engine(link, vid_id, title or vid_id, is_video)
        if result:
            return result, True
